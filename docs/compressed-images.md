...
The compressed folder uses a naming convention of `<subject>-<width>x<height>.ext`. The site uses three standard sizes:
...
Two additional formats exist in this folder but fall outside the standard keys (see "Off-standard formats" below).
...
**Component behavior:** Full-bleed `object-cover` image filling the hero container. Large source images perform best.

| Slot | Current original | Ideal compressed replacement |
|---|---|---|
| `image` | `top5pct-glitter-t-shirts.jpg` (800x600) | No 1:1 match. `glitter-shirt-crest-hill-1920x1080.jpg` is the only hero-scale compressed image. Verify it is the same or an acceptable subject before swapping. |
...
| 6 | `top5pct-custom-glitter-birthday-shirts-joliet.jpg` (800x600) | None | None | No match — different naming convention from `gitter-birthday-shirt` |
...
**Component behavior:** Renders at `width:600px; height:450px; max-width:100%` per site standard.
...
**Component behavior:** Same as above.
...
**Component behavior:** Full-bleed `absolute inset-0 w-full h-full object-cover`. Needs a large source image.

| Slot | Current original | Compressed replacement |
|---|---|---|
| `image` | `top5pct-custom-wedding-glitter-shirts.jpg` (800x600) | No match — not compressed yet |
...
|---|---|---|
| `glitter-shirt-crest-hill-400x300.jpg` | 400x300 | "shirt" singular — different from the "shirts" plural versions below |
...
| `glitter-shirts-crest-hill-400x300.jpg` | 400x300 | "shirts" plural — likely a different crop or photo than the singular version |
...
These files use dimensions or formats outside the three standard keys. They may be intended for future components or specialty placements.

| File | Actual dimensions | Ratio | Notes |
|---|---|---|---|
| `glitter-shirt-crest-hill-800x1200.webp` | 800x1200 | 2:3 portrait | Tall portrait format — not suited for current 4:3 sections |
| `glitter-shirts-webp.webp` | 800x1200 | 2:3 portrait | Same format; generic name |
| `glitter-shirt-joliet-spurs-basketball-400x300.jpg` | 400x267 | ~3:2 | Off-ratio — the actual image is not square 4:3 |
| `glitter-shirtt-joliet-spurs-basketball-600x450.jpg` | 600x400 | 3:2 | Typo in filename ("shirtt"); also off-ratio |

---

## Originals With No Compressed Counterpart

These original images are in the blade or in the source folder but have no matching file in the compressed folder. They still need to be compressed if they are to follow the standard.

| Original file | Used in blade | Dimensions |
|---|---|---|
| `top5pct-glitter-t-shirts.jpg` | Yes — `category-hero` | 800x600 |
| `top5pct-custom-glitter-clothing.jpg` | Yes — carousel slot 1 | 800x600 |
| `top5pct-custom-glitter-birthday-shirts-joliet.jpg` | Yes — carousel slot 6 | 800x600 |
| `top5pct-custom-glitter-sweatshirts.jpg` | Yes — carousel slot 7 | 800x600 |
| `top5pct-custom-wedding-glitter-shirts.jpg` | Yes — `card-banner-slide-in` | 800x600 |
| `top5pct-glitter-sportswear-shirts.jpg` | Yes — carousel slot 3 | 800x600 |
| `top5pct-custom-glitter-shirt-cap-hoodie-joliet.jpg` | No | 400x485 |
| `top5pct-custom-glitter-sweatshirts-joliet.jpg` | No (duplicate subject covered by `top5pct-glitter-sweatshirts-joliet.jpg`) | 800x600 |

---

## Likely Misplaced File

`foil-shirts-cresthill-400x300.jpg` and `foil-shirts-cresthill-600x450.jpg` have a "foil-shirts" prefix and appear to belong to the foil-shirts page rather than the glitter-shirts page. They are currently sitting in `public/images/glitter-shirts-compressed/` and have no reference in the glitter-shirts blade.
