...
| 5 | **Each LP category banner must use an image from the sub-category dir it links to.** For example, the "Puff Print" banner (linking to `/custom-apparel/specialty-materials/puff-shirts`) must pull from `public/images/puff-shirts/`. Using a `custom-shirts/` or other generic dir fallback is only acceptable when the sub-category has no dedicated image dir yet. That fallback must be replaced once the sub-page migration creates the dedicated dir. |
